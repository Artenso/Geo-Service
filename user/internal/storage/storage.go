...
// Create adds user
func (r *storage) Create(ctx context.Context, user *model.User) (int64, error) {
...
func (r storage) GetByID(ctx context.Context, id int64) (*model.User, error) {
	builder := sq.Select("*").
		From(table).
		Where(sq.Eq{
			idCol: id,
...
func (r storage) List(ctx context.Context, limit, offset int64) ([]*model.User, error) {
	builder := sq.Select("*").
		From(table).
		Limit(uint64(limit)).
...
func (r storage) Delete(ctx context.Context, id int64) error {
